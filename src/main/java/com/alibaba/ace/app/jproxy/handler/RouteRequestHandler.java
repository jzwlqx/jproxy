 ...
 * @author jjz
 */
public class RouteRequestHandler extends AbstractRequestHandler {
    private static final Log                  log             = LogFactory
                                                                      .getLog(RouteRequestHandler.class);
    ...
    @Override
    protected void process(HttpRequest req, OutputStream os) throws Exception {
        ...
        if (p == null) {
            log.warn("No match proxy for " + url);
        } else {
            if (log.isDebugEnabled()) {
                log.debug("Use proxy " + p + " for " + url);
            }
        }

        SimpleRequestHandler handler = new SimpleRequestHandler(p);
        handler.process(req, os);
    }
